					...
					<table>
						<tr>
							<td height="2" width="2"><a
								href="Reservation?cd=<%=rs.getString("codehotel")%>&db=<%=request.getParameter("db")%>&df=<%=request.getParameter("df")%>&nbch=<%=request.getParameter("nbch")%>"><img
									alt="" src="<%=rs.getString("image")%>" /> </a></td>
							<hr>
							<td valign="top">Déscription :<br><%=rs.getString("description_hotel")%>
								<br>Adresse :<%=rs.getString("adresse")%>, <%=rs.getString("codepostale")%>
								- <%=rs.getString("ville")%><br> Numero de telephone pour
								plus d'information :&nbsp;&nbsp;<%=rs.getString("numtel")%>
							</td>
							<hr>
						</tr>
						<hr>
